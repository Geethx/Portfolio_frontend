...
const Experience = () => {
    ...
    return (
        <section id="experience" className="py-16 lg:py-24 relative" ref={ref}>
            <div className="max-w-[1200px] mx-auto px-6">
                ...
                <div className="relative max-w-[800px] mx-auto">
                    {/* Timeline line */}
                    <div className="absolute left-5 top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary-600 via-emerald-500 to-transparent" />

                    {experiences.map((exp, i) => (
                        <motion.div
                            key={`${exp.company}-${exp.role}`}
                            className="relative pl-[60px] mb-10 last:mb-0"
                        ...
                        >
                            {/* Dot */}
                            <div
                                className="absolute left-[13px] top-7 w-4 h-4 rounded-full z-2 ring-2 ring-white/10"
                            ...
                            <div className="glass-card glass-card-hover p-7">
                                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                                    <span
                                        className="text-xs font-semibold uppercase tracking-wider px-3 py-1 border rounded-full"
                                        style={{ color: exp.color, borderColor: `${exp.color}44` }}
                                    >
                                        {exp.type}
                                    </span>
                                    <span className="flex items-center gap-1.5 font-mono text-[0.8rem] text-slate-500">
                                        <FiCalendar /> {exp.period}
                                    </span>
                                </div>

                                <h3 className="text-[1.2rem] font-bold mb-1.5">{exp.role}</h3>
                                <p className="flex items-center gap-1.5 text-[0.9rem] font-semibold mb-4" style={{ color: exp.color }}>
                                    <FiBriefcase /> {exp.company}
                                </p>

                                <ul className="flex flex-col gap-2">
                                    {exp.description.map((item, j) => (
                                        <li key={j} className="flex items-start gap-2.5 text-[0.88rem] text-slate-400 leading-[1.6]">
                                            <span className="w-1.5 h-1.5 min-w-[6px] rounded-full mt-2" style={{ background: exp.color }} />
                                            {item}
                                        </li>
                                    ))}
                                </ul>
